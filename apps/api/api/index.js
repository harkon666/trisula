...
// ../../node_modules/.bun/drizzle-orm@0.45.1+7b666ef5e701cd79/node_modules/drizzle-orm/sql/functions/aggregate.js
...
var redeemStatusEnum = pgEnum("redeem_status", [
  "pending",
  "processing",
  "ready",
  "completed",
...
var agentActivationCodes = pgTable("agent_activation_codes", {
...
var loginLogs = pgTable("login_logs", {
  id: serial("id").primaryKey(),
  userId: uuid("user_id").references(() => users.id).notNull(),
  loginDate: date("login_date").defaultNow()
});
var polisData = pgTable("polis_data", {
  id: serial("id").primaryKey(),
...
var waInteractions = pgTable("wa_interactions", {
  id: serial("id").primaryKey(),
...
var announcementViews = pgTable("announcement_views", {
  id: serial("id").primaryKey(),
...
auth.post("/register/agent", zValidator("json", RegisterAgentSchema), async (c) => {
  ...
  try {
    return await db.transaction(async (tx) => {
      const [code] = await tx.select().from(agentActivationCodes).where(and(
      ...
      await tx.update(agentActivationCodes).set({ isUsed: true, usedBy: newUser.id }).where(eq(agentActivationCodes.id, code.id));
...
auth.post("/register/nasabah", zValidator("json", RegisterNasabahSchema), async (c) => {
  ...
  try {
    return await db.transaction(async (tx) => {
      const [agent] = await tx.select().from(users).where(and(eq(users.userId, referredByAgentId), eq(users.role, "agent"))).limit(1);
      if (!agent) {
...
auth.post("/login", zValidator("json", LoginSchema), async (c) => {
  ...
  try {
    ...
    const [userWithProfile] = await db.select({
      user: users,
      profile: profiles
    }).from(users).leftJoin(profiles, eq(users.id, profiles.userId)).where(or(
    ...
    return c.json({
      ...
      user: {
        ...
        // Reflect updated balance if bonus awarded
      }
    });
  } catch (error) {
...
user.get("/profile", async (c) => {
  ...
  try {
    ...
    }).from(users).leftJoin(profiles, eq(users.id, profiles.userId)).where(eq(users.id, contextUser.id)).limit(1);
    if (!userData) {
...
user.get("/activity", async (c) => {
  ...
  try {
    ...
    }).from(pointsLedger).where(eq(pointsLedger.userId, contextUser.id)).orderBy(desc(pointsLedger.createdAt)).limit(50);
    return c.json({
      success: true,
      data: history
    });
...
user.get("/my-referrals", async (c) => {
  ...
  try {
    ...
    }).from(users).leftJoin(profiles, eq(users.id, profiles.userId)).where(eq(profiles.referredByAgentId, contextUser.userId)).orderBy(desc(users.createdAt));
    return c.json({
      success: true,
      data: list
    });
...
redeem.get("/my-requests", async (c) => {
  ...
  try {
    ...
    }).from(redeemRequests).leftJoin(rewards, eq(redeemRequests.rewardId, rewards.id)).where(eq(redeemRequests.nasabahId, user2.id)).orderBy(desc(redeemRequests.createdAt));
...
redeem.post("/", zValidator2("json", RedeemInputSchema), async (c) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const [userRecord] = await tx.select().from(users).where(eq(users.id, user2.id)).limit(1);
      const currentBalance = userRecord?.pointsBalance || 0;
      if (currentBalance < item.requiredPoints) {
        throw new Error("Poin tidak mencukupi");
      }
      await tx.update(users).set({ pointsBalance: sql`${users.pointsBalance} - ${item.requiredPoints}` }).where(eq(users.id, user2.id));
      await tx.insert(pointsLedger).values({
        userId: user2.id,
...
redeem.post("/:id/cancel", zValidator2("json", CancelInputSchema), async (c) => {
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      }).from(redeemRequests).leftJoin(rewards, eq(redeemRequests.rewardId, rewards.id)).where(eq(redeemRequests.id, requestId)).limit(1);
      if (!request) {
      ...
      await tx.update(users).set({ pointsBalance: sql`${users.pointsBalance} + ${pointCost}` }).where(eq(users.id, user2.id));
      await tx.insert(pointsLedger).values({
        userId: user2.id,
        amount: pointCost,
        source: "refund",
...
admin.get("/redeem/pending", async (c) => {
  try {
    const requests = await db.select({
      ...
      status: redeemRequests.status,
      createdAt: redeemRequests.createdAt
    }).from(redeemRequests).leftJoin(users, eq(redeemRequests.nasabahId, users.id)).leftJoin(profiles, eq(users.id, profiles.userId)).leftJoin(rewards, eq(redeemRequests.rewardId, rewards.id)).where(inArray(redeemRequests.status, ["pending", "processing", "ready"])).orderBy(desc(redeemRequests.createdAt));
    return c.json({ success: true, data: requests });
  } catch (error) {
  ...
  try {
    return await db.transaction(async (tx) => {
      ...
      if (status === "rejected") {
        ...
        await tx.update(users).set({ pointsBalance: sql`${users.pointsBalance} + ${pointsToRefund}` }).where(eq(users.id, request.userId));
        await tx.insert(pointsLedger).values({
...
admin.get("/codes", async (c) => {
  try {
    ...
    }).from(agentActivationCodes).leftJoin(users, eq(agentActivationCodes.generatedBy, users.id)).leftJoin(profiles, eq(agentActivationCodes.usedBy, profiles.userId)).orderBy(desc(agentActivationCodes.createdAt));
...
admin.delete("/codes/:id", async (c) => {
  ...
  try {
    const [deleted] = await db.delete(agentActivationCodes).where(sql`${agentActivationCodes.id} = ${codeId} AND ${agentActivationCodes.isUsed} = false`).returning();
    if (!deleted) {
...
productsRoute.get("/", async (c) => {
  try {
    const items = await db.select().from(products).where(eq(products.isActive, true)).orderBy(desc(products.id));
    return c.json({ success: true, data: items });
  } catch (error) {
...
productsRoute.post("/", rbacMiddleware(), zValidator4("json", CreateProductSchema), async (c) => {
...
productsRoute.patch("/:id", rbacMiddleware(), zValidator4("json", UpdateProductSchema), async (c) => {
...
productsRoute.delete("/:id", rbacMiddleware(), async (c) => {
  ...
  if (isNaN(id)) return c.json({ success: false, message: "Invalid ID" }, 400);
  try {
    const [deleted] = await db.update(products).set({ isActive: false }).where(eq(products.id, id)).returning();
    if (!deleted) {
      return c.json({ success: false, message: "Product not found" }, 404);
    }
...
polisRoute.post("/", rbacMiddleware(), zValidator5("json", InputPolisSchema), async (c) => {
...
polisRoute.get("/", rbacMiddleware(), async (c) => {
  try {
    ...
    }).from(polisData).leftJoin(users, eq(polisData.nasabahId, users.id)).leftJoin(profiles, eq(users.id, profiles.userId)).orderBy(desc(polisData.createdAt)).limit(100);
...
polisRoute.get("/my-polis", rbacMiddleware(), async (c) => {
  ...
  try {
    ...
    }).from(polisData).where(or(
      eq(polisData.nasabahId, user2.id),
      eq(polisData.agentId, user2.id)
...
async function listAnnouncements(c) {
  const user2 = c.get("user");
  try {
    const items = await db.select().from(announcements).where(eq(announcements.isActive, true)).orderBy(desc(announcements.id));
    let result = items;
    if (user2) {
      const views = await db.select().from(announcementViews).where(eq(announcementViews.userId, user2.id));
...
contentRoute.post("/announcements", rbacMiddleware(), zValidator6("json", CreateAnnouncementSchema), async (c) => {
  const body = c.req.valid("json");
  try {
...
contentRoute.post("/announcements/:id/view", rbacMiddleware(), async (c) => {
  ...
  if (isNaN(id)) return c.json({ success: false, message: "Invalid ID" }, 400);
  try {
    const [existing] = await db.select().from(announcementViews).where(and(
      eq(announcementViews.announcementId, id),
      eq(announcementViews.userId, user2.id)
    )).limit(1);
    if (!existing) {
...
contentRoute.patch("/announcements/:id", rbacMiddleware(), zValidator6("json", z6.object({ isActive: z6.boolean() })), async (c) => {
  const id = parseInt(c.req.param("id"));
  const { isActive } = c.req.valid("json");
  try {
    const [updated] = await db.update(announcements).set({ isActive }).where(eq(announcements.id, id)).returning();
...
monitoringRoute.post("/watchdog", async (c) => {
  ...
  try {
    ...
    const staleInteractions = await db.select().from(waInteractions).where(and(
    ...
    for (const item of staleInteractions) {
      await db.update(waInteractions).set({ isAdminNotified: true }).where(eq(waInteractions.id, item.id));
    }
    return c.json({
      success: true,
...
dashboardRoute.get("/stats", rbacMiddleware(), async (c) => {
  ...
  try {
    const [
      ...
      db.select({ total: sql`sum(${users.pointsBalance})` }).from(users)
    ]);
    const recentActions = await db.select().from(adminActions).orderBy(desc(adminActions.createdAt)).limit(5);
    return c.json({
      success: true,
      data: {
...
var vercel_default = handle(src_default2);
export {
  config,
