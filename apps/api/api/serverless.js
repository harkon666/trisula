    ...
    Ai = I((Dh, _i) => {
      ...
      var Aa = a(function(r) {
        ...
        r(
          ...
          1082,
          lt2
        ), r(1114, lt2), r(1184, lt2), r(600, er), r(651, ne), r(718, Ia), r(1e3, Ea), r(1001, Ta), r(
          1005,
          Yt
...
var RegisterAgentSchema = z2.object({
  password: z2.string().min(6),
  fullName: z2.string().min(2),
  email: z2.string().email("Email tidak valid"),
  whatsapp: z2.string().min(10),
  activationCode: z2.string().min(5),
  dateOfBirth: z2.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Format tanggal salah (YYYY-MM-DD)")
});
var RegisterNasabahSchema = z2.object({
  password: z2.string().min(6),
  fullName: z2.string().min(2),
  email: z2.string().email("Email tidak valid"),
  whatsapp: z2.string().min(10),
  referredByAgentId: z2.string().min(4),
...
auth.post("/login", zValidator("json", LoginSchema), async (c) => {
  ...
  try {
    ...
    const [userWithProfile] = await db.select({
      ...
      profile: profiles
    }).from(users).leftJoin(profiles, eq2(users.id, profiles.userId)).where(or2(
...
var RedeemInputSchema = z3.object({
  rewardId: z3.number().int(),
  whatsappNumber: z3.string().min(10).optional()
...
var CancelInputSchema = z3.object({
...
var UpdateRedeemStatusSchema = z4.object({
  status: z4.enum(["processing", "ready", "completed", "rejected"]),
...
admin.post("/codes", zValidator3("json", z4.object({
  code: z4.string().min(3, "Kode minimal 3 karakter")
})), async (c) => {
  const user2 = c.get("user");
...
admin.post("/rewards", zValidator3("json", z4.object({
  title: z4.string().min(3),
  description: z4.string().optional(),
  requiredPoints: z4.number().int().positive(),
  csWhatsappNumber: z4.string().optional(),
...
admin.patch("/rewards/:id", zValidator3("json", z4.object({
  title: z4.string().min(3).optional(),
  description: z4.string().optional(),
  requiredPoints: z4.number().int().positive().optional(),
  csWhatsappNumber: z4.string().optional(),
...
admin.patch("/users/:id/metadata", zValidator3("json", z4.object({
...
var CreateAnnouncementSchema = z4.object({
  title: z4.string().min(1, "Judul wajib diisi"),
  content: z4.string().optional(),
  videoUrl: z4.string().optional(),
  ctaUrl: z4.string().optional(),
...
var UpdateAnnouncementSchema = z4.object({
...
internal.get("/nasabah-agents", rbacMiddleware("polis"), async (c) => {
  try {
    ...
    const nasabahWithAgents = await db.transaction(async (tx) => {
      ...
      const agentMap = new Map(allAgents.map((a2) => [a2.userId, a2]));
      return list.map((n) => ({
        ...n,
...
var CreateProductSchema = z5.object({
  name: z5.string().min(3),
  description: z5.string().optional(),
  pointsReward: z5.number().int().nonnegative(),
...
import { eq as eq9, desc as desc6, or as or3 } from "drizzle-orm";
var polisRoute = new Hono7();
var InputPolisSchema = z6.object({
  polisNumber: z6.string().min(3),
  nasabahId: z6.string().uuid(),
  // UUID of the user (Nasabah)
  agentId: z6.string().uuid(),
  // UUID of the user (Agent)
  premiumAmount: z6.number().int().positive()
});
...
var CreateAnnouncementSchema2 = z7.object({
  ...
  content: z7.string().min(10),
  videoUrl: z7.string().url().optional(),
...
async function listAnnouncements(c) {
  ...
  try {
    ...
    if (user2) {
      ...
      const viewedIds = new Set(views.map((v2) => v2.announcementId));
...
contentRoute.patch("/announcements/:id", rbacMiddleware("announcements"), zValidator6("json", z7.object({ isActive: z7.boolean() })), async (c) => {
  const id = parseInt(c.req.param("id"));
...
monitoringRoute.post("/interaction", rbacMiddleware(), zValidator7("json", z8.object({
...
var LogWaSchema = z9.object({
  productId: z9.number().int(),
...
agentRoute.get("/reminders", rbacMiddleware(), async (c) => {
  ...
  try {
    ...
    const now = /* @__PURE__ */ new Date();
    const reminders = agentPolis.map((p2) => {
      if (!p2.createdAt) return null;
      let months = (now.getFullYear() - p2.createdAt.getFullYear()) * 12;
      months -= p2.createdAt.getMonth();
      months += now.getMonth();
      ...
      if (monthsLeft === 1 || monthsLeft === 2 || monthsLeft === 3) {
        return {
          ...p2,
          monthsLeft,
          message: `Polis #${p2.polisNumber} atas nama ${p2.nasabahName} jatuh tempo dalam ${monthsLeft} bulan.`
        };
      }
      return null;
    }).filter(Boolean);
...
// src/serverless.ts
var runtime = "edge";
var serverless_default = index_default;
export {
